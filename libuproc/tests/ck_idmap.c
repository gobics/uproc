...
#include <check.h>
#include "uproc.h"
#undef UPROC_CLASS_MAX
#define UPROC_CLASS_MAX 100
#include "../idmap.c"
...
START_TEST(test_exhaust)
{
    uproc_class fam, i;
    char foo[1024];
    for (i = 0; i < UPROC_CLASS_MAX; i++) {
        sprintf(foo, "%" UPROC_CLASS_PRI, i);
        fam = uproc_idmap_class(map, foo);
        ck_assert_int_eq(fam, i);
    }
    fam = uproc_idmap_class(map, "test1");
    ck_assert_int_eq(fam, UPROC_CLASS_INVALID);
    fam = uproc_idmap_class(map, "test2");
    ck_assert_int_eq(fam, UPROC_CLASS_INVALID);
    fam = uproc_idmap_class(map, "test3");
    ck_assert_int_eq(fam, UPROC_CLASS_INVALID);
}
END_TEST
