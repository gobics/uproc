...
static void print_desc_literal(FILE *stream, char *desc)
{
    char *s = desc;
    fputc('\n', stream);
    for (; *s; s++) {
...
static void print_desc(FILE *stream, char *desc, int indent, int wrap)
{
    char *s = desc;
...
#if HAVE_GETOPT_LONG
                fprintf(stream, FMT_LONG, w_long, opt->longopt, w_arg,
                        opt->argname);
#endif
                fprintf(stream, "%*s", SPACE_BEFORE_DESC, "");
                if (strchr(opt->desc, '\n')) {
...
int main(void)
{
    ...
    ppopts_add(&opt, 'o', "option", "ARG",
               "Some long and very descriptive text. I hope it is long enough "
               "to wrap");
    ppopts_add(&opt, 'l', "longeroption", "LONGERARG",
               "Another long and very descriptive text. I hope it is long "
               "enough to wrap even if we use PPOPTS_DESC_ON_NEXT_LINE.");
    ppopts_add(
        &opt, 's', "super", "FOO",
        "This is another supercalifragilisticexpialidocious option! Yeah");
    ppopts_add_text(&opt, "FOO should be < %d", 3);
    ppopts_add(&opt, 'n', "newline", "X",
               "This option contains a newline:\nit shall always be printed "
               "literally (including   all  \t whitespace).");
