...
#define TEST(X, L, U, RES) do {                                 \
    ...
    TEST(0, 1, 1, UPRO_LOOKUP_OOB);
    TEST(1, 1, 1, UPRO_LOOKUP_EXACT);
    TEST(2, 1, 3, UPRO_LOOKUP_INEXACT);
    TEST(3, 3, 3, UPRO_LOOKUP_EXACT);
    TEST(4, 3, 5, UPRO_LOOKUP_INEXACT);
    TEST(9, 5, 10, UPRO_LOOKUP_INEXACT);
    TEST(42, 10, 44, UPRO_LOOKUP_INEXACT);
    TEST(44, 44, 44, UPRO_LOOKUP_EXACT);
    TEST(131, 131, 131, UPRO_LOOKUP_EXACT);
    TEST(133, 133, 133, UPRO_LOOKUP_EXACT);
    TEST(134, 133, 1202, UPRO_LOOKUP_INEXACT);
#undef TEST
    return SUCCESS;
}
...
#define TEST(X, LP, UP, IDX, COUNT, RES) do {                            \
    ...
    TEST(0, 3, 3,       0, 1, UPRO_LOOKUP_OOB);
    TEST(3, 3, 3,       0, 2, UPRO_LOOKUP_EXACT);
    TEST(5, 3, 31,      1, 2, UPRO_LOOKUP_INEXACT);
    TEST(31, 31, 31,    2, 3, UPRO_LOOKUP_EXACT);
    TEST(32, 31, 53,    4, 2, UPRO_LOOKUP_INEXACT);
    TEST(52, 31, 53,    4, 2, UPRO_LOOKUP_INEXACT);
    TEST(53, 53, 53,    5, 2, UPRO_LOOKUP_EXACT);
    TEST(99, 99, 99,    7, 2, UPRO_LOOKUP_EXACT);
    TEST(100, 99, 99,   8, 1, UPRO_LOOKUP_OOB);
#undef TEST
    return SUCCESS;
}
...
#define TEST(WP, WS, LP, LS, UP, US, RES) do {                              \
    ...
    TEST(0, 1,  3, 1,  3, 1,  UPRO_LOOKUP_OOB);
    TEST(0, 44, 3, 1,  3, 1,  UPRO_LOOKUP_OOB);

    TEST(3, 1,  3, 1,  3, 1,  UPRO_LOOKUP_EXACT);
    TEST(3, 2,  3, 1,  3, 3,  UPRO_LOOKUP_INEXACT);

    TEST(31, 1,  31, 5,  31, 5,  UPRO_LOOKUP_INEXACT);
    TEST(31, 7,  31, 5,  31, 10,  UPRO_LOOKUP_INEXACT);
    TEST(31, 12,  31, 10,  31, 44,  UPRO_LOOKUP_INEXACT);

#define A (4551ULL << (3 * UPRO_AMINO_BITS))
    TEST(UPRO_PREFIX_MAX, 1,  99, A, 99, A, UPRO_LOOKUP_OOB);
    return SUCCESS;
}
...
int test_append(void)
{
    ...
    res = upro_ecurve_append(&a1, &a2);
    assert_int_eq(res, UPRO_EINVAL, "appending again failed with UPRO_EINVAL");
