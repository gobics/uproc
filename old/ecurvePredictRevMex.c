/*
 ============================================================================
 * function: [PredScoresComb,PredCountsComb,PredPfamsComb,PredSeqIs,PredBegs,PredEnds] = ... 
        ecurvePredictRevMex(single(ScoresComb), uint16(PfamsComb), uint32(SeqPos), ScoreMatComb, Thresholds);
 * desc.: combined forward/reverse scoring and prediction
 *
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])	{
    ...
    unsigned int indScore, indCount, indLength, posiOld, posiNew, pfamMax, count, countMax, iRow, nRows, nThres, offsetNew, offsetOld, nPosis;
    unsigned int PfamPredInds[N_PFAMS], PredIndsOld[N_PREDS_MAX], PredPosisBeg[N_PREDS_MAX], PredPosisEnd[N_PREDS_MAX], PredPosiOld[N_PREDS_MAX], PredOffsOld[N_PREDS_MAX];
    ...
    for(iSeq=0; iSeq<nSeqs; iSeq++){
        ...
        if(seqLen>=WLEN){
            ...
            /* second loop: prepare new candidate predictions for final scoring using pfams with max single-match-score>0 */
            ...
            nPosis = (seqEnd-seqBeg+1)/2; /* half forward, half reverse - check for valid seqEnd (SeqBegins!) in matlab call!!  */
            /* mexPrintf("nPosis: %d\n", nPosis); */

            for(iScore=seqBeg; iScore<=seqEnd-nPosis; iScore++){ /* only first half loop with offset access to second half */
                for(offsetNew=0; offsetNew<=nPosis; offsetNew+=nPosis){
                    pfam = Pfams[iScore + offsetNew];
                    if(pfam > 0){
                        indPfam = pfam-1;
                        ...
                        if(iPred > 0){
                            iPred -= 1;
                            posiNew = iScore;
                            if(PredCountsNew[iPred]>0){ /* if Old-values of previous match (with that pfam) exist*/
                                offsetOld = PredOffsOld[iPred]; /*PredOffOld[iPred];*/ /* zero if forward, offset=nRows*nScores if reverse */
                                ...
                                for(iRow=0; iRow<nRows-overlap; iRow++)
                                    PredScoresNew[iPred] += ScoreMat[(offsetOld+posiOld)*nRows + iRow];
            ...
            for(iPred=0; iPred<nPredsNew; iPred++){
                ...
                posiOld = PredPosiOld[iPred];
                offsetOld = PredOffsOld[iPred];
                scoreSum = PredScoresNew[iPred];
                for(iRow=0; iRow<nRows; iRow++)
                    scoreSum += ScoreMat[(offsetOld+posiOld)*nRows + iRow];
