...
void
ec_seq_filter_init(struct ec_seq_filter *filter,
                     enum ec_seq_filter_flags flags,
                     enum ec_seq_filter_mode default_mode,
                     const char *accept, const char *ignore, const char *warn)
{
    ...
    for (p = accept; p && *p; p++) {
        filter->mode[(unsigned char) *p] = EC_SEQ_ACCEPT;
    }
    for (p = ignore; p && *p; p++) {
        filter->mode[(unsigned char) *p] = EC_SEQ_IGNORE;
    }
    for (p = warn; p && *p; p++) {
        filter->mode[(unsigned char) *p] = EC_SEQ_WARN;
    }
    filter->flags = flags;
...
int
ec_seq_fasta_read(FILE *stream, struct ec_seq_filter *filter,
...
#define GROW_BUF(buf, sz) do {                              \
    ...
    /* reset `filter` counts */
    if (filter && (filter->flags & EC_SEQ_F_RESET)) {
        filter->ignored = filter->warnings = 0;
    }
    ...
    while ((c = toupper(fgetc(stream))) != EOF) {
        ...
        /* apply filter */
        if (filter) {
            enum ec_seq_filter_mode f = filter->mode[(unsigned char) c];
            if (f == EC_SEQ_IGNORE) {
                filter->ignored++;
                continue;
            }
            else if (f == EC_SEQ_WARN) {
                filter->warnings++;
            }
        }
...
void
ec_seq_fasta_print(FILE *stream, const char *id, const char *comment,
